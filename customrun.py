...
class CustomEvents:
        ...
        def __init__(calling_instance, file_name, verbose=2):
                self.tick_events = load_custom_run_file(calling_instance, file_name, verbose)
        ...
        def load_custom_run_file(tickfile):
            if tickFile:
                if tickFile.endswith(".xml"):
                    load_custom_run_xml(self,tickFile)
                else:
                    print "Custom run event file name:", tickFile, " does not appear to be an xml file, please use an appropriate file ending."
            return tick_events
