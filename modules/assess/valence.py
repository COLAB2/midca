...
class PredicateChangeAssessor:
	...
	def window_valence(self, starti = None):
		states = self.mem.get(midca_inst.MEM_STATES)
		if starti == None:
		...
		prednames = self.mem.get(midca_inst.MEM_ADIST)[0].trace.prednames
	...
	def run(self, verbose = 2):
		if not self.mem.get(midca_inst.MEM_ANOM) or not self.mem.get(midca_inst.MEM_ANOM)[-1]:
			if verbose >= 3:
		...
		valence = self.characterize_change(self.mem.get(midca_inst.MEM_ANOM)[-1], self.window_valence(), self.baseline)
		self.mem._update(midca_inst.MEM_VALENCE, valence)
		if verbose >= 1:
