...
def putoutfire(state, b, ext):
	if state.fire[b] == True and state.holdingfireext == ext and state.fire_ext_charges[ext] > 0:
		state.fire[b] == False
                state.fire_ext_charges[ext] -= 1
		return state
	else: 
		return False

def pickup_extinguisher(state, extinguisher):
	if extinguisher in state.fire_ext_avail and state.fire_ext_charges[extinguisher] > 0 and not state.holdingfireext:
