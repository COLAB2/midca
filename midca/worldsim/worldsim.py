...
class World:
    ...
    def copy(self):
        ...
        return World(self.operators.values(), self.predicates.values(), self.atoms.copy(), self.types.copy(), self.objects.values(), self.cltree.copy(), self.obtree.copy())
    ...
    def apply(self, simAction):
        for i in range(len(simAction.results)):
            if simAction.postPos[i]:
                ...
                if self.atom_true(simAction.results[i]):
