...
def set_location(state, objectOrID, newLoc):
	state.sighting(world_repr.DetectionEvent(id = objectOrID,loc = newLoc))
	return state
...
def clear_block(state, depth, task):
        ...
        if m: 
            ...
            block_on_top_of_this_block,location_top_block = clear_block_monitor.monitor_clear_block_asus(b1)
            if block_on_top_of_this_block:
                print("monitor fires!!")
                m.is_fired = True 
                set_clear_status(state, b1, 'not clear')
                set_position(state, block_on_top_of_this_block, b1)
		set_location(state,block_on_top_of_this_block,location_top_block)
