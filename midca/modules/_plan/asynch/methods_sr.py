        ...
        if goal["objective"] == "holding":
            ...
            if not get_status_in_arm(state):
                ...
                if get_last_clear_status(state, object) == 'clear':
                    ...
                    return [("move_one", get_max_height_position_block(state, object), 'table'),
                            ("achieve_goals", [goal]), ("achieve_goals", goals[1:])]

            else:

                return [("move_one", get_status_in_arm(state), 'table'), ("achieve_goals", [goal]),
...
def status(b1, state, goal):
    ...
    if status_in_arm:
        ...
        if b1 == direct_object:
            ...
            if (not last_position == 'table') and (not (last_position == indirect_object)) and (
            not (get_last_clear_status(state, indirect_object) == 'clear')):
                return 'move-to-table'
...
def move1(state, b1, dest):
    ...
    if get_last_position(state, b1) == "in-arm":
        ...
        return [('unstack_task', get_top_status(state, b1)), ('putdown', get_top_status(state, b1)),
                ('release', get_top_status(state, b1)), ('get', b1), ('put', b1, dest)]
    elif get_last_clear_status(state, dest) == 'not clear':
        return [('unstack_task', get_top_status(state, dest)), ('putdown', get_top_status(state, dest)),
                ('release', get_top_status(state, dest)), ('get', b1), ('put', b1, dest)]
