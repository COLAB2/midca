...
class PyHopPlanner(base.BaseModule):
    ...
    pyhop_state_from_world_restaurant = None
    pyhop_tasks_from_goals_restaurant = None
    
    def __init__(self,
                 pyhop_state_from_world_restaurant,
                 pyhop_tasks_from_goals_restaurant,
                 declare_methods,
                 declare_operators,
                 extinguishers = False,
                 mortar = False):
        
        self.pyhop_state_from_world_restaurant = pyhop_state_from_world_restaurant
        self.pyhop_tasks_from_goals_restaurant = pyhop_tasks_from_goals_restaurant

    def __init__(self):
        #declares pyhop methods. This is where the planner should be given the domain information it needs.
            restaurant_methods.declare_methods()
            restaurant_operators.declare_ops()
    #this will require a lot more error handling, but ignoring now for debugging.
    def run(self, cycle, verbose = 2):
        world = self.mem.get(self.mem.STATES)[-1]
        goals = self.mem.get(self.mem.CURRENT_GOALS)
        ...
        if not midcaPlan:
            ...
            try:
                pyhopState = pyhop_state_from_world_restaurant(world)
            except Exception:
                print "Could not generate a valid pyhop state from current world state. Skipping planning"
            try:
                pyhopTasks = pyhop_tasks_from_goals_restaurant(goals)
            except Exception:
                print "Could not generate a valid pyhop task from current goal set. Skipping planning"
            try:
                pyhopPlan = pyhop.pyhop(pyhopState, pyhopTasks, verbose = 0)
