...
class EvalPointingFromFeedback(base.BaseModule):
    def run(self, cycle, verbose=2):
        ...
        if not goals:
            ...
            if not plan:
                if verbose >= 2:
                    print("No plan found that achieves all current goals. ","Skipping eval based on plan completion")
            else:
                if plan.finished():
                    try:
...
class Scorer:
    ...
    def run(self, cycle, verbose=2):
        ...
        if verbose >= 2:
            print("Tower", self.mem.get(SCORE).towers, "completed.", score, "added to score, which is now", self.mem.get(
                SCORE).score)
...
class MortarScore:
    ...
    def __str__(self):
        return "towers=" + str(self.towers) + ",score=" + str(self.score) + ",mortarblocks=" + str(
            self.mortarblocks) + ",regularblocks=" + str(self.regularblocks)
...
class MortarScorer:
    ...
    def get_all_stacking_goals(self):
        ...
        for goal in self.mem.get(self.mem.CURRENT_GOALS)[-1]:
            ...
            if 'predicate' in goal and (goal['predicate'] == 'on' or goal['predicate'] == 'stable-on') and goal.args[
                0] == 'D_':  # TODO this should just automatically figure out the highest block in the tower, but this is assuming D is always the highest, which in Intend, it will always choose goals with 'D' on top
                return goal
        return None
    ...
    def run(self, cycle, verbose=2):
        ...
        if verbose >= 2:
            print(
            "Tower", self.mem.get(
                MORTARSCORE).towers, "completed.", score, "added to score, which is now", self.mem.get(
                MORTARSCORE).score)
...
class Evaluator:
    ...
    def run(self, verbose=2):
        ...
        if currentgoals:
            ...
            if verbose >= 2:
                print(
                "No current goal. Skipping Eval.")
        if world.is_true("on", ["D_", "C_"]):
            if verbose >= 2:
                print(
                "Tall Tower completed")
            ...
            if verbose >= 2:
                print(
                "Short Tower completed")
            self.towersFinished += 1
            self.score += (3 - self.num_fires_small(world)) * NORMAL_BLOCK_VAL + self.num_fires_small(
                world) * ON_FIRE_BLOCK_VAL
            if self.restartFires:
                self.put_out_fires(verbose)
        if verbose >= 2:
            print(
            "Total towers built:", self.towersFinished)
            print(
            "Total fire turns:", self.fireturns)
            print(
        ...
        if accomplished and currentPlan:
            if verbose >= 2:
                print(
                "Goal completed for current plan. Removing all intended goals and pending plans for this goal.")
...
class NBeaconsDataRecorder:
    ...
    def newrun(self, cycle, verbose=2):
        ...
        try:
            ...
            print(
            "unable to check goal", currentGoal, ". skipping scoring")
        ...
        if at_least_one_achieved and all_achieved:
            ...
            self.mem.set(self.mem.GOALS_ACTIONS_ACHIEVED,
                         goal_action_pairs + [(last_pair[0] + 1, actions_executed_thus_far)])
        ...
        if goals:
            for goal in goals:
                try:
                    ...
                    if verbose >= 1:
                        print(
                        "Could not test goal", goal, ". It does not seem to be a valid world state")
                    return
            if verbose >= 1:
                print(
                "All current goals achieved. Removing them from goal graph")
            ...
            if all_goals_achieved:
                ...
                for goal in goals:
                    if 'activated' in str(goal):
                        ...
                        actions_executed_thus_far = self.mem.get(self.mem.ACTIONS_EXECUTED)
                        if self.verbose >= 1:
                            print(" just added goals actions pair: " +
                                  str((goals_achieved, str(goal.args[0]), actions_executed_thus_far)))

                        self.mem.set(self.mem.GOALS_ACTIONS_ACHIEVED, goal_action_pairs + [
                            (goals_achieved, str(goal.args[0]), actions_executed_thus_far)])
        ...
        if currentgoals:
            ...
            for currentgoal in currentgoals:
                ...
                if currentgoal.goaltype == "activated":
                    ...
                    print(("accomplishedthis = " + str(accomplishedthis) + " for activated(" + str(
                        [arg.id for arg in currentgoal.goalargs])))
                accomplished = accomplished and accomplishedthis
