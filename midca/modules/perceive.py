...
class MoosObserverMultiagent(base.BaseModule):
    '''
    MIDCA Module which interacts with moos to get the current states
    of the vehicle in the moos.
    '''
    ...
    def get_the_location(self, current_position, states, vehicle):
        ...
        transit1 = [-25, -43]
        transit2 = [135, -44]
        qroute_transit = [41, -119]

        #should check if the vehicle is in Q-route1, Q-route2, GA1, GA2
        if y >=qroute_y[0] and y<=qroute_y[1]:
                states+="passed(" + vehicle + ",transit1)\n"
                states+="at_location(" + vehicle + ",qroute)\n"
                if (x >= qroute_transit[0]-5 and y >= qroute_transit[1]-5) and (x <= qroute_transit[0]+5 and y <= qroute_transit[1]+5):
                    states+="at_location(" + vehicle + ",qroute_transit)\n"
                    states+="passed(" + vehicle + ",qroute_transit)\n"
                elif (x > ga1[0]-5 and x<= ga1[0]+5) and (y > ga1[1]-5 and y<= ga1[1]+5) :
                        #if (x == 28) and (y == -62) :
                        states+="at_location(" + vehicle + ",ga1)\n"
                elif (x > ga2[0]-5 and x<= ga2[0]+5) and (y > ga2[1]-5 and y <=ga2[1]+5):
                        states+="at_location(" + vehicle + ",ga2)\n"
        elif (x >= transit1[0]-5 and y >= transit1[1]-5) and (x <= transit1[0]+5 and y <= transit1[1]+5):
                states+="at_location(" + vehicle + ",transit1)\n"
                states+="passed(" + vehicle + ",transit1)\n"
        elif (x >= transit2[0]-5 and y >= transit2[1]-5) and (x <= transit2[0]+5 and y <= transit2[1]+5):
                states+="at_location(" + vehicle + ",transit2)\n"
        elif x>home[0]-5 and y > home[1]-5:
                states+="at_location(" + vehicle + ",home)\n"
        else:
            states+="at_location(" + vehicle + ",transit)\n"
...
class GraceObserver(base.BaseModule):
    ...
    def init(self, world, mem):
        ...
        self.subscriber_remus.setsockopt(zmq.RCVTIMEO, -1)
        self.subscriber_remus.setsockopt(zmq.CONFLATE, 1)
        self.subscriber_remus.bind("tcp://127.0.0.1:9563")
    ...
    def get_the_location(self, current_position, states, vehicle):
        ...
        if vehicle == "grace":
            self.mem.set(self.mem.AGENT_LOCATION, [x,y])
            self.mem.set(self.mem.REMUS_SPEED, float(speed))
            loc = self.sim_to_grid(x,y)
            states+="agent-at(" + vehicle + "," + loc +")\n"
    ...
    def run(self, cycle, verbose=2):
        ...
        try:
            current_position = self.subscriber_remus.recv()
            states += self.get_the_location(current_position, states, "grace")
        except:
            print ("Remus location not recieved")
