...
import math
from midca import goals as make_goals
...
class SimpleIntendFlairs(base.BaseModule):
    ...
    def get_all_recently_detected_mines(self, location):
            """
    ...
    def random(self):
        if not self.forrandom:
            explanation_goals = self.mem.get(self.mem.SELECT_EXPLANATION_GOALS)
            random.seed(self.seed)
            if explanation_goals:
                goals = [random.choice(explanation_goals)]
                self.mem.set(self.mem.SELECT_EXPLANATION_GOALS, None)
                self.forrandom = 1
                return goals


    def select(self):
        time_left = self.deadline - (midcatime.now() - self.mem.get(self.mem.MOOS_TIME))
    ...
    def choose(self, explanation_goals):
        """

        :param goals: goals from goal graph
        :return:
        """


        if explanation_goals:
            #explanation_goals = explanation_goals[0]
            for explanation_goal in explanation_goals:
                if explanation_goal:
                    if explanation_goal['predicate'] == "apprehended":
                        apprehended = 2
    ...
    def run(self, cycle, verbose = 2):
        ...
        """
        if self.mem.get(self.mem.SELECT_EXPLANATION_GOALS):
            explanation_goals = self.mem.get(self.mem.SELECT_EXPLANATION_GOALS)
            selected_goals = self.select(explanation_goals)
            for goal in selected_goals:
                explanation_goals.remove(goal)
            self.mem.set(self.mem.SELECT_EXPLANATION_GOALS, explanation_goals)
        """
