...
def singlerun(args):
    ...
    if agent_type == 'm':
        print()
        "Running Meta agent for " + str(META_NUM_CYCLES) + " cycles"
        ...
        print()
        "Running GDA agent for " + str(GDA_NUM_CYCLES) + " cycles"
        ...
        print()
        "Running Vanilla agent for " + str(VANILLA_NUM_CYCLES) + " cycles"
...
class MIDCAInstance():
    ...
    def create_vanilla_MIDCA(self):
        ...
        myMidca.append_module("Simulate", simulator.NBeaconsActionSimulator(wind=WIND_ENABLED, wind_dir=self.wind_dir,
                                                                            wind_strength=self.wind_strength,
                                                                            dim=DIMENSION,
                                                                            wind_schedule=WIND_SCHEDULE_1,
                                                                            logfilenm=DATADIR + "vanilla" + NOW_STR + ".log"))
    ...
    def create_gda_MIDCA(self):
        ...
        myMidca.append_module("Simulate", simulator.NBeaconsActionSimulator(wind=WIND_ENABLED, wind_dir=self.wind_dir,
                                                                            ...
                                                                            dim=DIMENSION,
                                                                            wind_schedule=WIND_SCHEDULE_1,
                                                                            logfilenm=DATADIR + "gda" + NOW_STR + ".log"))
        myMidca.append_module("Simulate", simulator.ASCIIWorldViewer(DISPLAY_FUNC))
        myMidca.append_module("Perceive", perceive.PerfectObserver())
    ...
    def create_meta_MIDCA(self):
        ...
        # Add phases by name
        for phase in ["Simulate", "Perceive", "Interpret1", "Interpret2", "Interpret3", "Eval", "Cleanup", "Intend",
                      "Plan", "Act"]:
            myMidca.append_phase(phase)
        ...
        myMidca.append_module("Simulate", simulator.NBeaconsActionSimulator(wind=WIND_ENABLED, wind_dir=self.wind_dir,
                                                                            ...
                                                                            dim=DIMENSION,
                                                                            wind_schedule=WIND_SCHEDULE_1,
                                                                            logfilenm=DATADIR + "meta" + NOW_STR + ".log"))
...
def multiple_goalsperaction(n_files):
    ...
    datafiles = [DATADIR + f for f in files[-(n_files):]]
        ...
        with open(df, 'r') as f:
            for line in f.readlines():
                if header:
                    ...
                    goals_achieved_data = [t[0] for t in goals_action_data]
                    actions_executed_data = [t[2] for t in goals_action_data]
                    ...
                    if agent_type == 'v':
                        agent_name = 'Vanilla'
                        print()
                        "  Finished processing vanilla data"
                    elif agent_type == 'g':
                        agent_name = 'GDA'
                        print()
                        "  Finished processing gda data"
                    elif agent_type == 'm':
                        agent_name = 'Meta'
                        print()
                        "  Finished processing meta data"
    ...
    for k, v in list(goal_action_final_val_vanilla.items()):
        print()
        "  Goal #" + str(k) + " has average execution cost " + str(v)
    ...
    ax.plot(list(goal_action_final_val_vanilla.keys()), list(goal_action_final_val_vanilla.values()), 'r--', label='Replanning',
            lw=4)
    ax.plot(list(goal_action_final_val_gda.keys()), list(goal_action_final_val_gda.values()), 'g-.', label='GDA', lw=4)
    ax.plot(list(goal_action_final_val_meta.keys()), list(goal_action_final_val_meta.values()), 'b:', label='Meta', lw=4)
    ax.set_xlim(0, 550)
    ax.set_ylim(0, 8000)
        ...
        with open(df, 'r') as f:
            for line in f.readlines():
                if header:
                    ...
                    ax.plot(goals_achieved_data, actions_executed_data, linestyles[agent_type],
                            label=agent_name + str(file_count), lw=2)
    ...
    with open(datafile, 'r') as f:
        for line in f.readlines():
            if header:
                ...
                ax.plot(goals_achieved_data, actions_executed_data, linestyles[line_style_index], label=agent_name,
                        lw=4)
                line_style_index += 1
        ...
        if len(sys.argv) > 2:
            if len(sys.argv) > 3:
                if str(sys.argv[2]) == 'multiple':
                    print()
                    "graphing an average of the last " + sys.argv[2] + " runs"
                    multiple_goalsperaction(int(sys.argv[3]))
                elif str(sys.argv[2]) == 'each':
                    print()
                    "graphing each of the last " + sys.argv[3] + " runs"
