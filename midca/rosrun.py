        ...
        if color_location_dic and ('red block' in color_location_dic.keys()) and (
            ...
            if math.fabs(color_location_dic['red block'].x - color_location_dic['green block'].x) < 12:
                ...
                self.mem.add(self.mem.ROS_OBJS_STATE,
                             world_repr.pos_block(id="red block", position=pos_red, isclear=clear_red))
                self.mem.add(self.mem.ROS_OBJS_STATE,
...
class threeObjectsLocationHandler(IncomingMsgHandler):
    ...
    def store_locations(self, data):
        ...
        if color_location_dic and ('red block' in color_location_dic.keys()) and (
            ...
            ydistance = [color_location_dic['red block'].y, color_location_dic['blue block'].y,
                         color_location_dic['green block'].y]
            xdistance = [color_location_dic['red block'].x, color_location_dic['blue block'].x,
                         color_location_dic['green block'].x]
            ...
            elif ((xdistancerg <= 13) and (xdistancerb <= 13) and (xdistancegb <= 13) and (ydistancerg <= 45) and (
                    ydistancerb <= 45) and (ydistancegb <= 45) and (distancerg <= 45) and (distancerb <= 45) and (
                          distancegb <= 45) and ((ydistancerg > 28.5) or (ydistancerb > 28.5) or (ydistancegb > 28.5))):
                add = 0
                for i in edistance:
                    add = add + i;
            ...
            elif (((xdistancerg <= 13) and (xdistancerb <= 13) and (xdistancegb <= 13)) ^ (
                    (ydistancerg <= 28.5) and (ydistancerb <= 28.5) and (ydistancegb <= 28.5) and (
                    distancerg <= 28.5) and (distancerb <= 28.5) and (distancegb <= 28.5)) or (
                          (xdistancerg <= 13) or (xdistancerb <= 13) or (xdistancegb <= 13)) and (
                          (ydistancerg <= 28.5) or (ydistancerb <= 28.5) or (ydistancegb <= 28.5))):
                if (xdistancerg <= 13):
                    temps = ydistance
            ...
            for each_block in color_location_dic:
                ...
                for cmp_block in color_location_dic:
                    if not each_block == cmp_block:
                        ...
                        if (
                                x_difference < 0.03 and y_difference < 0.01 and z_difference > 0.02 and z_difference < 0.06):
                ...
                for each_history in self.mem.get(self.history):
                    ...
                    for each_object in objects:
                        if not each_object in color_location_dic:
                            self.mem.add(self.mem.ROS_OBJS_STATE,
                                         world_repr.pos_block(id=each_object, position='holding', isclear='not clear'))
                            found = 1
                            break
                    if found == 1:
                        break
