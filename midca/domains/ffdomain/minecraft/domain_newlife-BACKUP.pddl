(define (domain minecraft-beta)
	...
	(:action attack-skeleton-hand
		:parameters (?tool - tool ?loc - mapgrid ?bow_tool - tool)
		:precondition
			(and
...
(:action attack-skeleton-with-loc
	...
	(:action attack-skeleton-with-loc-hand
		:parameters ( ?loc - mapgrid ?player_loc - mapgrid ?tool - tool ?bow_tool - tool)
		:precondition
			(and
			    (> (player-current-health) 0)
	...
	(:action destroy-trap-hand
		:parameters (?tool - tool ?loc - mapgrid ?axe_tool - tool)
		:precondition
			(and

                (> (player-current-health) 0)
                (= (tool-id ?axe_tool) 11)
				(< (thing-available ?axe_tool) 1)
			    (player-at ?loc)
				(known-loc arrowtrap ?loc)
				(thing-at arrowtrap ?loc)
				(= (tool-id ?tool) 0)
...
;;----------------------------------
    (:action destroy-trap-with-loc-hand
		:parameters (?loc - mapgrid ?player_loc - mapgrid ?tool - tool ?axe-tool - tool)
		:precondition
			(and
...
;;---------------------------------------------------------------
	...
	(:action harvest-hand
		:parameters (?target - mapgrid ?tool - tool ?obj - resource ?axe_tool - tool)
		:precondition
			(and
			 (> (player-current-health) 0)
				(player-at ?target)
