...
class NBeaconGrid():
    ...
    def get_inner_tile_grid(self):
        ...
        if len(self.TILE_GRID) > 0:
            ...
            print
            "please call generate_tiles() first"
            return False
...
class Tile():
    ...
    def set_adjacent_to(self, dest):
        ...
        if dest.getX() < 0 or dest.getY() < 0:
            raise Exception(
                "Tile " + str(dest) + " has invalid coordinates: x " + dest.getX() + " and y " + dest.getY())
        ...
        if ((xdiff != 0 and ydiff != 0) or
            ...
            raise Exception(
                "Tile " + str(dest) + " is not adjacent, xdiff is " + str(xdiff) + ", ydiff is " + str(ydiff))
        ...
        if xdiff != 0:
            if xdiff < 0:
                ...
                if self.eastTile:
                    raise Exception(
                        "Error: trying to set eastTile to " + str(dest) + " but eastTile already set to " + str(
                            self.eastTile))
                self.eastTile = dest
                ...
                if self.westTile:
                    raise Exception(
                        "Error: trying to set westTile to " + str(dest) + " but westTile already set to " + str(
                            self.westTile))
                self.westTile = dest
        else:
            if ydiff > 0:
                ...
                if self.southTile:
                    raise Exception(
                        "Error: trying to set southTile to " + str(dest) + " but southTile already set to " + str(
                            self.southTile))
                self.southTile = dest
            else:
                ...
                if self.northTile:
                    raise Exception(
                        "Error: trying to set northTile to " + str(dest) + " but northTile already set to " + str(
                            self.northTile))
                self.northTile = dest
