...
class coloredBlocksworld/ros/coloredBlocksworldEntitiesHandler(IncomingMsgHandler):
    ...
    def __init__(self, topic, midcaObject, memKey = None, history = None):
        ...
        msgType = String
		self.left = None
        super(coloredBlocksworld/ros/coloredBlocksworldEntitiesHandler, self).__init__(topic, msgType, callback, midcaObject)
        #self.objID = objID
        if memKey:
            self.memKey = memKey
        else:
