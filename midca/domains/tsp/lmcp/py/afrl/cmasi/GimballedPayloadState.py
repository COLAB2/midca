...
class GimballedPayloadState(PayloadState.PayloadState):
    ...
    def __init__(self):
        ...
        self.FULL_LMCP_TYPE_NAME = "afrl.cmasi.GimballedPayloadState"
    ...
    def toDict(self):
        ...
        if (not "CMASI") or (len("CMASI") == 0): # this should never happen
            ...
            d["datatype"] = str("DEBUG_PROBLEM_HERE" + "/GimballedPayloadState")
            d["datastring"] = str(m)
        else:
            d['datatype'] = str("CMASI" + "/GimballedPayloadState")
            d['datastring'] = str(m)
        return d
