...
class PayloadConfiguration(LMCPObject.LMCPObject):
    ...
    def __init__(self):
        ...
        self.FULL_LMCP_TYPE_NAME = "afrl.cmasi.PayloadConfiguration"
    ...
    def pack(self):
        ...
        if len(self.PayloadKind) > 0:
            if (sys.version_info > (3, 0)):
                buffer.extend(struct.pack( repr(len(self.PayloadKind)) + "s", self.PayloadKind.encode('utf-8')))
            else:
    ...
    def unpack(self, buffer, _pos):
        ...
        if _strlen > 0:
            if (sys.version_info > (3, 0)):
                self.PayloadKind = struct.unpack_from( repr(_strlen) + "s", buffer, _pos )[0].decode('utf-8')
            else:
    ...
    def unpackFromXMLNode(self, el, seriesFactory):
        ...
        for e in el.childNodes:
            if e.nodeType == xml.dom.Node.ELEMENT_NODE:
                ...
                elif e.localName == "PayloadKind" :
                    self.PayloadKind = str(e.childNodes[0].nodeValue) if len(e.childNodes) > 0 else ""
    ...
    def unpackFromDict(self, d, seriesFactory):
        ...
        for key in d:
            ...
            elif key == "PayloadKind":
                self.PayloadKind = d[key]
            elif key == "Parameters":
                self.Parameters = []
                for c in d[key]:
    ...
    def toDict(self):
        ...
        if (not "CMASI") or (len("CMASI") == 0): # this should never happen
            ...
            d["datatype"] = str("DEBUG_PROBLEM_HERE" + "/PayloadConfiguration")
            d["datastring"] = str(m)
        else:
            d['datatype'] = str("CMASI" + "/PayloadConfiguration")
            d['datastring'] = str(m)
        return d
    ...
    def toDictMembers(self, d):
        ...
        d['PayloadKind'] = self.PayloadKind
        d['Parameters'] = []
        for x in self.Parameters:
            if x == None:
