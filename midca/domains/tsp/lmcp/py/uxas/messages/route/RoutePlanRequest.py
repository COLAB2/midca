...
class RoutePlanRequest(LMCPObject.LMCPObject):
    ...
    def toDict(self):
        ...
        if (not "ROUTE") or (len("ROUTE") == 0): # this should never happen
            ...
            d["datatype"] = str("DEBUG_PROBLEM_HERE" + "/RoutePlanRequest")
            d["datastring"] = str(m)
        else:
            d['datatype'] = str("ROUTE" + "/RoutePlanRequest")
            d['datastring'] = str(m)
        return d
