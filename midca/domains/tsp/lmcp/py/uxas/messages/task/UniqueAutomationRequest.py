...
class UniqueAutomationRequest(LMCPObject.LMCPObject):

    def __init__(self):

        self.LMCP_TYPE = 8
        self.SERIES_NAME = "UXTASK"
        self.FULL_LMCP_TYPE_NAME = "uxas.messages.task.UniqueAutomationRequest"
    ...
    def toString(self):
        ...
        buf += "From UniqueAutomationRequest:\n"
    ...
    def toDict(self):
        ...
        if (not "UXTASK") or (len("UXTASK") == 0): # this should never happen
            ...
            d["datatype"] = str("DEBUG_PROBLEM_HERE" + "/UniqueAutomationRequest")
            d["datastring"] = str(m)
        else:
            d['datatype'] = str("UXTASK" + "/UniqueAutomationRequest")
            d['datastring'] = str(m)
        return d
