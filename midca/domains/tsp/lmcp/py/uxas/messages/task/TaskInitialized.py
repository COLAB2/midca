...
class TaskInitialized(LMCPObject.LMCPObject):

    def __init__(self):

        self.LMCP_TYPE = 26
        self.SERIES_NAME = "UXTASK"
        self.FULL_LMCP_TYPE_NAME = "uxas.messages.task.TaskInitialized"
    ...
    def toDict(self):
        ...
        if (not "UXTASK") or (len("UXTASK") == 0): # this should never happen
            ...
            d["datatype"] = str("DEBUG_PROBLEM_HERE" + "/TaskInitialized")
            d["datastring"] = str(m)
        else:
            d['datatype'] = str("UXTASK" + "/TaskInitialized")
            d['datastring'] = str(m)
        return d
