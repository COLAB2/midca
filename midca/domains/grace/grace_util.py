...
def parse_tile(input):
    ...
    output += input[2:y_index] + " " + input[y_index+1:]
    return output
...
#     STATE_FILE = "C:/Users/Zohreh/git/MIDCA/modules/_plan/jShop/problem"
    f = open(STATE_FILE, 'w')
    f.write('\n')
    f.write("(defproblem problem travel (\n")
    f.write("\n")
...
#     STATE_FILE = "C:/Users/Zohreh/git/MIDCA/modules/_plan/jShop/problem"
    ...
    for goal in goals:
        ...
        for i in range(len(args)):
            if args[i].startswith("Tx"):
                args[i] = parse_tile(args[i])
...
def asciiframestr(frame, numbered_borders = False):
    result = ""
    if numbered_borders:
        result +="   "
...
def drawNBeaconsScene(midcastate,rtn_str=False):
    ...
    for beaconkey in pyhopState.activated.keys():
        ...
        if pyhopState.activated[beaconkey]:
            if pyhopState.beaconlocs[beaconkey] == pyhopState.agents['grace']:
...
def drawFishTags(midcastate):

    pyhopState = pyhop_state_from_world_grace(midcastate)

    print ("-----------Grace World------------------\n")
    drawNBeaconsScene(midcastate)

    print ("\n-----------Fish Tag Detections------------------\n")
    # initialize the grid with list of empty list
    dim = pyhopState.dim['dim']
    grid = []
    dirt = "."
    for c in range(dim):
        row = []
        for r in range(dim):
            row.append([dirt, dirt, dirt, dirt, dirt])

        grid.append(row)

    for atom in midcastate.atoms:
        if atom.predicate.name == "uniqueTagCount":
            tag_count = int(atom.args[1].name)

            # sub areas
            sub_area = int(atom.args[2].name.replace("subarea-", "")) - 1

            # location
            loc = [int(loc) for loc in parse_tile(atom.args[3].name).split(" ")]

            grid[loc[1]][loc[0]][sub_area] = tag_count

    drawlistinlists(grid)
...
def check_agent_loc(world, loc):

    agentatlocation = False

    # check agent tile
    for atom in world.atoms:
        if atom.predicate.name == "agent-at":
            if atom.args[1].name == loc:
                agentatlocation = True
            else:
                agentatlocation = False
            break

    return agentatlocation

def check_if_communicated(world, loc):

    agentatlocation = False

    # check agent sub-area
    for atom in world.atoms:
        if atom.predicate.name == "communicated-hotspot":
            if atom.args[2].name == loc:
                agentatlocation = True
            else:
                agentatlocation = False
            break
...
def monitor(world, mem, plan, verbose=2):

    index = len([action for action in plan if (action.status == asynch_grace.COMPLETE)])
    if plan:
        ...
        monitor_to_check = ["hotspot-detected"]
        addedActions = None
        for atom in world.atoms:
            # check if the predicate in monitors
            if atom.predicate.name in monitor_to_check:
                #check if the agent is at the location
                if check_agent_loc(world, atom.args[1].name) and \
                    ...
                    addedActions = [plans.Action("ascend", "grace", atom.args[1].name),
                                    ...
                                    plans.Action("descend", "grace", atom.args[1].name)]
                    if verbose >=2:
                        print ("\n Monitor Fired because of state: " + str(atom))
                        print ("Added action :" + str(addedActions))
        if addedActions:
                plan.actions.insert(index, asynch_grace.GraceAscend(mem, addedActions[0]))
                plan.actions.insert(index+1, asynch_grace.GraceCommunicate(mem, addedActions[1]))
                plan.actions.insert(index+2, asynch_grace.GraceDescend(mem, addedActions[2]))
