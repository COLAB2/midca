...
namespace sac{			  
	...
	float EIDVal(Eigen::Matrix< double, erg_dim, 1 > & x){
		...
		float alpha = .4;
		double pos[] = {x[0]*scale+offset_x, x[1]*scale+offset_y};
		float val;
		float mu=gp.f(pos);
...
/* iSACstep() function operator
  ...
  iSACstep.u_switch - vector of SAC action values applied from [t_i, t_f] which is a subset of [t0, t0+ts].
          If [t_i, t_f] is not equal to [t0, t0+ts] then the default control is applied over the remaining 
interval.
...
{											 
	...
	double t0 = t_init;//std::stof(holder[xlen],&sz);
	bool run = true;
	while (run){
	//while (t0 < 60 ){
		std::string msg;
		...
		if (holder.front().compare("change_scale")==0){
			cout << buff << " changing scale \n";
			scale =std::stof (holder[1],&sz);
			msg = "confirm";
			send(new_socket,msg.c_str(),msg.size(),MSG_NOSIGNAL);
			continue;
		}
		if (holder.front().compare("offset")==0){
			cout << buff << " setting offsets \n";
			offset_x =std::stof (holder[1],&sz);
			offset_y =std::stof (holder[2],&sz);
			msg = "confirm";
			send(new_socket,msg.c_str(),msg.size(),MSG_NOSIGNAL);
			continue;
		}
		if (holder.front().compare("read_gp")==0){
			cout << buff << " reading gp\n";
			x =std::stof (holder[1],&sz);
			y =std::stof (holder[2],&sz);
			double pos[] = {x,y};
			msg = std::to_string(gp.f(pos))+","+std::to_string(gp.var(pos));
			send(new_socket,msg.c_str(),msg.size(),MSG_NOSIGNAL);
			continue;
		}
		...
		msg = std::to_string(iSACstep.u_switch[0])+","+std::to_string(iSACstep.u_switch[1]);
		send(new_socket,msg.c_str(),msg.size(),MSG_NOSIGNAL);
	    ...
	    for (i=0; i < xlen; ++i) { x0[i] = std::stof (holder[i],&sz);}//cout << i <<","<< holder[i] << "\n";}
		double pos[] = {x0[0]*scale+offset_x, x0[1]*scale+offset_y};
		...
		if (!updateMeas) {
			...
			if (outOfBounds){
				...
				if (updateCount%25==0){
					...
					float amu=0;
					maxVariance=0;
	                for(i=0;i<res;i++){for(int j=0;j<res;j++){
						double p[] = {i*scale/res+.5*scale/res+offset_x, j*scale/res+.5*scale/res+offset_y};avar= gp.var(p);
						if(maxVariance<avar){maxVariance=avar;}}
					}
					if(maxVariance==0){maxVariance=1;}
					for(i=0;i<res;i++){
						for(int j=0;j<res;j++){
							double p[] = {i*scale/res+.5*scale/res+offset_x, j*scale/res+.5*scale/res+offset_y};
			...
			if (!isnan(x0[0]) && !isnan(x0[1]) && !isnan(rate)){
                gp.add_pattern(pos, rate);
			}
			cout << "measurement num = " <<updateCount << ", maxMean = " << maxMean<< ", maxVariance = " << maxVariance<< ", average variance = " << averageVariance << "\n";
		}
	}
	//get full field scale simulation
	scale = fullscale;
    float res = 40.0;
	...
	float amu=0;
	maxVariance=0;
	for(i=0;i<=res;i++){for(int j=0;j<=res;j++){double p[] = {i*scale/res+0*scale/res, j*scale/res+0*scale/res};
	avar= gp.var(p);
	if(maxVariance<avar){maxVariance=avar;}}}
    for(i=0;i<=res;i++){
		for(int j=0;j<=res;j++){
