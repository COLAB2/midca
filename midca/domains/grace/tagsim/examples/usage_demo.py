...
'''
example code showing how to simulate agents in tagfield
'''
def draw(x):
    plt.figure(1)
    plt.axis('scaled')
...
#plt.plot_surface(pnts[:,:,0], pnts[:,:,1], np.flip(np.round(approx,decimals=2),(0,1)).transpose(), 20, cmap='coolwarm')# cmap='inferno'), cmap='RdGy')
