...
def pyhop_state_from_world(world, name = "state"):
    s = pyhop.State(name)
    s.agents={'grace':'3,3'} # put beacons here too? and mud tiles?
    s.dim={'dim':5}
    s.agents = {}
...
#     STATE_FILE = "C:/Users/Zohreh/git/MIDCA/modules/_plan/jShop/problem"
    ...
    for goal in goals:
        ...
        if args[0] == predicate:
            args.pop(0)
        if predicate == "agent-at":
            f.write("(agent-at " + args[0] + " " +parse_tile(args[1])+")\n")
        if predicate == "communicated":
            f.write("(agent-at " + args[0] + " " +parse_tile(args[3])+")\n")
        else:
